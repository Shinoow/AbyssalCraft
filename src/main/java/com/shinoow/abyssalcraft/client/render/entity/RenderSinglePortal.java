...
public class RenderSinglePortal extends Render<EntitySinglePortal> {

	private static final ResourceLocation texture = new ResourceLocation("abyssalcraft", "textures/model/black_hole.png");

	public RenderSinglePortal(RenderManager manager){
		super(manager);
	}

	@Override
	public void doRender(EntitySinglePortal entity, double x, double y, double z, float entityYaw, float partialTicks)
	{
		entity.clientTicks++;

		float wobbleScale = 0.05f;
		float wobble = entity.clientTicks / 10f;
		float wobbleX = (float) (Math.sin(wobble) * wobbleScale) + 0.6f;
		float wobbleY = (float) (Math.sin(wobble) * -1 * wobbleScale) + 0.6f;

		GlStateManager.pushMatrix();
		GlStateManager.translate(x, y + 1, z);
		if(entity.ticksExisted <= 30) {
			float f = entity.ticksExisted / 45f;
	...
	@Override
	protected ResourceLocation getEntityTexture(EntitySinglePortal entity) {
