...
public class EntropySpell extends Spell {
	...
	public boolean canCastSpell(World world, BlockPos pos, EntityPlayer player) {
		...
		if(r != null && r.typeOfHit == Type.BLOCK) return getDegradation(world.getBlockState(r.getBlockPos())) != null;
		return false;
	}
	...
	private IBlockState getDegradation(IBlockState state){
		...
		else if(state.getBlock() == ACBlocks.darkstone_cobblestone || state.getBlock() == ACBlocks.abyssal_cobblestone ||
			state.getBlock() == ACBlocks.coralium_cobblestone || state.getBlock() == ACBlocks.dreadstone_cobblestone ||
			state.getBlock() == ACBlocks.abyssalnite_cobblestone || state.getBlock() == Blocks.COBBLESTONE)
