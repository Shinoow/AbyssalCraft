...
public class BlockRitualAltar extends BlockSingletonInventory {
	...
	public void randomDisplayTick(IBlockState state, World par1World, BlockPos pos, Random par5Random) {
		...
		if(ACConfig.particleBlock){
			...
			if(altar instanceof IRitualAltar && ((IRitualAltar) altar).isPerformingRitual())
				flag = true;
			if(flag) {
				AbyssalCraft.proxy.spawnParticle("BlueFlame", pos.getX() + 0.75, pos.getY() + 1.05, pos.getZ() + 0.75, 0.0D, 0.0D, 0.0D);
				AbyssalCraft.proxy.spawnParticle("BlueFlame", pos.getX() + 0.25, pos.getY() + 1.05, pos.getZ() + 0.75, 0.0D, 0.0D, 0.0D);
				AbyssalCraft.proxy.spawnParticle("BlueFlame", pos.getX() + 0.25, pos.getY() + 1.05, pos.getZ() + 0.25, 0.0D, 0.0D, 0.0D);
				AbyssalCraft.proxy.spawnParticle("BlueFlame", pos.getX() + 0.75, pos.getY() + 1.05, pos.getZ() + 0.25, 0.0D, 0.0D, 0.0D);
			} else {
	...
	public enum EnumRitualMatType implements IStringSerializable {
		...
		ABYSSAL_STONE_BRICK(2, "abyssal_stone_brick", 0x2a4a2d),
		CORALIUM_STONE_BRICK(3, "coralium_stone_brick", 0x2e4d44),
		DREADSTONE_BRICK(4, "dreadstone_brick", 0x7a2020),
		ELYSIAN_STONE_BRICK(5, "elysian_stone_brick", 0x2e3348),
