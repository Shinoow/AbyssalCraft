		...
		if (!par5Entity.isRiding() && !par5Entity.isBeingRidden() && !par1World.isRemote)
			if(par5Entity.timeUntilPortal <= 0){
				if(par5Entity instanceof EntityPlayerMP){
					...
					if (thePlayer.dimension != ACLib.omothol_id)
					{
						...
						thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, ACLib.abyssal_wasteland_id, new TeleporterAC(thePlayer.mcServer.worldServerForDimension(ACLib.abyssal_wasteland_id), this, ACBlocks.omothol_stone));
					}
				} else {
					MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance();
					par5Entity.timeUntilPortal = par5Entity.getPortalCooldown();

					if(par5Entity.dimension != ACLib.omothol_id){
						if(!ForgeHooks.onTravelToDimension(par5Entity, ACLib.omothol_id)) return;

						int i = par5Entity.dimension;

						par5Entity.dimension = ACLib.omothol_id;
						par1World.removeEntityDangerously(par5Entity);

						par5Entity.isDead = false;

						server.getPlayerList().transferEntityToWorld(par5Entity, i, server.worldServerForDimension(i), server.worldServerForDimension(ACLib.omothol_id), new TeleporterAC(server.worldServerForDimension(ACLib.omothol_id), this, ACBlocks.omothol_stone));
					} else {
						...
						server.getPlayerList().transferEntityToWorld(par5Entity, ACLib.omothol_id, server.worldServerForDimension(ACLib.omothol_id), server.worldServerForDimension(ACLib.dreadlands_id), new TeleporterAC(server.worldServerForDimension(ACLib.dreadlands_id), this, ACBlocks.omothol_stone));
					}
				}
