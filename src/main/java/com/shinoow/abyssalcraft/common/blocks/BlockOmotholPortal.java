...
public class BlockOmotholPortal extends BlockBreakable
{
	...
	{
		super("OG", Material.portal , false);
	...
	{
		blockIcon = par1IconRegister.registerIcon(AbyssalCraft.modid + ":" + "OG");
	}
	...
	public static boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		byte b0 = 0;
		byte b1 = 0;
		if (par1World.getBlock(par2 - 1, par3, par4) == AbyssalCraft.omotholstone || par1World.getBlock(par2 + 1, par3, par4) == AbyssalCraft.omotholstone)
			b0 = 1;
		if (par1World.getBlock(par2, par3, par4 - 1) == AbyssalCraft.omotholstone || par1World.getBlock(par2, par3, par4 + 1) == AbyssalCraft.omotholstone)
			b1 = 1;
		if (b0 == b1)
			return false;
		else
			...
			for (l = -1; l <= 2; ++l)
				for (i1 = -1; i1 <= 3; ++i1)
				{
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
					{
						...
						if (flag)
						{
							if (j1 != AbyssalCraft.omotholstone)
								return false;
						}
						else if (j1 != Blocks.air && j1 != AbyssalCraft.omotholfire)
							return false;
					}
				}
			for (l = 0; l < 2; ++l)
				for (i1 = 0; i1 < 3; ++i1)
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, AbyssalCraft.omotholportal, 0, 2);
			return true;
		}
	}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, Block par5)
	{
		...
		for (i1 = par3; par1World.getBlock(par2, i1 - 1, par4) == this; --i1)
			;
		if (par1World.getBlock(par2, i1 - 1, par4) != AbyssalCraft.omotholstone)
		...
		else
		{
			...
			for (j1 = 1; j1 < 4 && par1World.getBlock(par2, i1 + j1, par4) == this; ++j1)
				;
			if (j1 == 3 && par1World.getBlock(par2, i1 + j1, par4) == AbyssalCraft.omotholstone)
			{
				boolean flag = par1World.getBlock(par2 - 1, par3, par4) == this || par1World.getBlock(par2 + 1, par3, par4) == this;
				boolean flag1 = par1World.getBlock(par2, par3, par4 - 1) == this || par1World.getBlock(par2, par3, par4 + 1) == this;
				if (flag && flag1)
					par1World.setBlockToAir(par2, par3, par4);
				else if ((par1World.getBlock(par2 + b0, par3, par4 + b1) != AbyssalCraft.omotholstone || par1World.getBlock(par2 - b0, par3, par4 - b1) != this) && (par1World.getBlock(par2 - b0, par3, par4 - b1) != AbyssalCraft.omotholstone || par1World.getBlock(par2 + b0, par3, par4 + b1) != this))
					par1World.setBlockToAir(par2, par3, par4);
			} else
				par1World.setBlockToAir(par2, par3, par4);
		}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
	{
		...
		if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null && par5Entity instanceof EntityPlayerMP)
		{
			...
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, AbyssalCraft.configDimId3, new TeleporterOmothol(thePlayer.mcServer.worldServerForDimension(AbyssalCraft.configDimId3)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, AbyssalCraft.configDimId2, new TeleporterOmothol(thePlayer.mcServer.worldServerForDimension(AbyssalCraft.configDimId2)));
			}
		}
	}
