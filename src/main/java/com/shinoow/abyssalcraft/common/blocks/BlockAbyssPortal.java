...
public class BlockAbyssPortal extends BlockBreakable {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	{
		...
		if (worldIn.provider.getDimension() != ACLib.abyssal_wasteland_id && worldIn.getGameRules().getBoolean("doMobSpawning") && rand.nextInt(2000) < worldIn.getDifficulty().getDifficultyId())
		{
			...
			{
				Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getKey(EntityAbyssalZombie.class), blockpos.getX() + 0.5D, blockpos.getY() + 1.1D, blockpos.getZ() + 0.5D);
