...
public class BlockDreadlandsPortal extends BlockBreakable {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	{
		...
		if (worldIn.provider.getDimension() != ACLib.dreadlands_id && worldIn.getGameRules().getBoolean("doMobSpawning") && rand.nextInt(2000) < worldIn.getDifficulty().getDifficultyId())
		{
			...
			{
				Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getEntityStringFromClass(EntityDreadSpawn.class), blockpos.getX() + 0.5D, blockpos.getY() + 1.1D, blockpos.getZ() + 0.5D);
