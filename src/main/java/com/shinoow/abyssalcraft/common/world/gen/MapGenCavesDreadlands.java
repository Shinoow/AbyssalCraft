...
public class MapGenCavesDreadlands extends MapGenCavesHell {
	...
	@Override
	protected void func_180704_a(long seed, int par2, int par3, ChunkPrimer primer, double x, double y, double z, float par8, float par9, float par10, int par11, int par12, double par13)
	{
		double d0 = par2 * 16 + 8;
		double d1 = par3 * 16 + 8;
		float f = 0.0F;
		...
		for (boolean flag = random.nextInt(6) == 0; par11 < par12; ++par11)
		{
			...
			{
				func_180704_a(random.nextLong(), par2, par3, primer, x, y, z, random.nextFloat() * 0.5F + 0.5F, par9 - (float)Math.PI / 2F, par10 / 3.0F, par11, par12, 1.0D);
				func_180704_a(random.nextLong(), par2, par3, primer, x, y, z, random.nextFloat() * 0.5F + 0.5F, par9 + (float)Math.PI / 2F, par10 / 3.0F, par11, par12, 1.0D);
				return;
			}
								...
								for (int i2 = l; i2 > k2; --i2)
								{
									...
									if (d9 > -0.7D && d10 * d10 + d9 * d9 + d8 * d8 < 1.0D)
									{
										...
										if (iblockstate1.getBlock() == ACBlocks.dreadstone || iblockstate1.getBlock() == ACBlocks.abyssalnite_stone || iblockstate1.getBlock() == ACBlocks.dreadlands_dirt || iblockstate1.getBlock() == ACBlocks.dreadlands_grass)
											primer.setBlockState(i3, i2, j3, field_186130_a);
									}
								}
							}
						}
