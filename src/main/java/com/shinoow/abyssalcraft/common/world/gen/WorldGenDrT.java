...
public class WorldGenDrT extends WorldGenTrees {
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		if (j1.getBlock() != Blocks.dirt && j1.getBlock() != ACBlocks.dreadstone && j1.getMaterial() != Material.grass || y >= 256 - height - 1)
			return false;
		...
		for (int b = 0; b < branches; b++) {
			...
			while (c < branchLenght) {
				...
				if(world.isAirBlock(new BlockPos(x + (int) (c * xd), y + (int) hd + 1, z + (int) (c * yd))))
					world.setBlockState(new BlockPos(x + (int) (c * xd), y + (int) hd + 1, z + (int) (c * yd)), ACBlocks.dreadlands_leaves.getStateFromMeta(12));
			}
		}
