...
public class WorldGenDeadTree extends WorldGenTreeAC {
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		for (int i = 0; i < height; i++)
			setBlockAndNotifyAdequately(world, new BlockPos(x, y + i, z), ACBlocks.dead_tree_log.getDefaultState());
		...
		for (int b = 0; b < branches; b++) {
			...
			while (c < branchLenght) {
				c++;
				hd += 0.5f;
				setBlockAndNotifyAdequately(world, new BlockPos(x + (int) (c * xd), y + (int) hd, z + (int) (c * yd)), ACBlocks.dead_tree_log.getStateFromMeta(12));
			}
		}
