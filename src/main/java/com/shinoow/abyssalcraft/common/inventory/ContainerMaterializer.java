...
public class ContainerMaterializer extends Container {
	...
	private int getRecipeAmount(){
		...
		for(int j = 2; j < 110; j++)
			if(tileMaterializer.getStackInSlot(j) == null){
				i = j;
				break;
			}
		...
		for (int k = 0; k < 3; ++k)
			for (int l = 0; l < 8; ++l)
			{
				...
				if (i1 >= 2 && i1 < getRecipeAmount()) basicInventory.setInventorySlotContents(l + k * 6, tileMaterializer.getStackInSlot(i1));
					...
					basicInventory.setInventorySlotContents(l + k * 6, null);
			}
	}
			...
			if (par2 != 1 && par2 != 0)
			{
				...
				if (par2 >= 2 && par2 < 20)
				{
					if (!mergeItemStack(itemstack1, 20, 56, false))
						return null;
					//					MaterializerRecipes.instance().processMaterialization(itemstack1, inventorySlots.get(0).getStack());
					slot.onSlotChange(itemstack1, itemstack);
				}
