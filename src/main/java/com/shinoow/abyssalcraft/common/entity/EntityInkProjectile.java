		...
		if (result.entityHit != null)
		{
			...
			if(result.entityHit instanceof EntityLivingBase && !worldObj.isRemote){
				if(rand.nextBoolean() && !EntityUtil.isEntityCoralium((EntityLivingBase) result.entityHit))
					((EntityLivingBase)result.entityHit).addPotionEffect(new PotionEffect(AbyssalCraftAPI.coralium_plague.id, 100));
				if(rand.nextInt(4) == 0)
					((EntityLivingBase)result.entityHit).addPotionEffect(new PotionEffect(Potion.blindness.id, 100));
				if(rand.nextInt(5) == 0)
					((EntityLivingBase)result.entityHit).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 100));
			}
