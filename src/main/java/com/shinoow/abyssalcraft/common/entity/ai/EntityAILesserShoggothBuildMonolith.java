...
public class EntityAILesserShoggothBuildMonolith extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if(shoggoth.isChild())
			...
			return false;
		else if(world.getEntitiesWithinAABB(shoggoth.getClass(), shoggoth.getEntityBoundingBox().expand(16D, 16D, 16D), e -> !e.equals(shoggoth) && !e.isAssisting && !e.isBuilding).size() < 3)
			return false;
		else {
			...
			if(vec3d == null)
				...
				List<EntityLesserShoggoth> list = world.getEntitiesWithinAABB(shoggoth.getClass(), shoggoth.getEntityBoundingBox().expand(16D, 16D, 16D), e -> !e.equals(shoggoth) && !e.isAssisting && !e.isBuilding);
				for(EntityLesserShoggoth shoggoth1 : list) {
					if(shoggoths.size() == 3)
						break;
					if(shoggoth1.getDistanceToEntity(shoggoth) <= 12) {
				...
				locationX = vec3d.xCoord;
				locationY = vec3d.yCoord;
				locationZ = vec3d.zCoord;
				return true;
			}
		}
	}
	...
	public void updateTask()
	{
		...
		if(shoggoth.getDistance(locationX, locationY, locationZ) <= 8 && allShoggothsNearby()) {
			...
			for(EntityLesserShoggoth shoggoth : world.getEntitiesWithinAABB(shoggoth.getClass(), shoggoth.getEntityBoundingBox().expand(32D, 32D, 32D)))
	...
	private Vec3d findPossibleLocation()
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			if (world.isAirBlock(blockpos1.up()) && world.isAirBlock(blockpos1.up(1)) && world.getBlockState(blockpos1).getBlock().isReplaceable(world, blockpos1)
					&& world.getBlockState(blockpos1.down()) != ACBlocks.monolith_stone
