...
public class EntityChagaroth extends EntityMob implements IDreadEntity {
	...
	protected void updateAITasks()
	{
		...
		for (int i = 1; i < 3; ++i)
			if (ticksExisted >= nextHeadUpdate[i - 1])
			{
				...
				{
					List<EntityLivingBase> list = world.<EntityLivingBase>getEntitiesWithinAABB(EntityLivingBase.class, getEntityBoundingBox().expandXyz(48D), Predicates.<EntityLivingBase>and(EntitySelectors.NOT_SPECTATING, entity -> !EntityUtil.isEntityDread(entity)));
	...
	public void onLivingUpdate()
	{
		...
		if (flameShootTimer > 0)
		{
			...
			if (ticksExisted % 5 == 0)
			{
				world.playSound(null, new BlockPos(posX + 0.5D, posY + getEyeHeight(), posZ + 0.5D), SoundEvents.ENTITY_GHAST_SHOOT, getSoundCategory(), 0.5F + getRNG().nextFloat(), getRNG().nextFloat() * 0.6F + 0.2F);
				world.playSound(null, new BlockPos(posX + 0.5D, posY + getEyeHeight(), posZ + 0.5D), SoundEvents.ENTITY_GHAST_SHOOT, getSoundCategory(), 0.5F + getRNG().nextFloat(), getRNG().nextFloat() * 0.5F + 0.2F);
				world.playSound(null, new BlockPos(posX + 0.5D, posY + getEyeHeight(), posZ + 0.5D), SoundEvents.ENTITY_GHAST_SHOOT, getSoundCategory(), 0.5F + getRNG().nextFloat(), getRNG().nextFloat() * 0.4F + 0.2F);
			}
			Entity target = getHeadLookTarget();
			if (target != null) {
				List list = world.getEntitiesWithinAABB(EntityLivingBase.class, target.getEntityBoundingBox().expand(2.0D, 2.0D, 2.0D), Predicates.and(new Predicate[] { EntitySelectors.IS_ALIVE }));
	...
	protected void onDeathUpdate()
	{
		...
		if(deathTicks % 20 == 0)
			for(Entity entity : world.getEntitiesInAABBexcluding(this, getEntityBoundingBox().expand(15, 15, 15), entity -> entity instanceof EntityDreadguard || entity instanceof EntityDreadSpawn
				...
				entity.attackEntityFrom(DamageSource.OUT_OF_WORLD, 20);
