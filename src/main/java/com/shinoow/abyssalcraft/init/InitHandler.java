...
public class InitHandler implements ILifeCycleHandler {
	...
	private static void syncConfig(){
		...
		cfg.setCategoryComment(CATEGORY_ITEM_BLACKLIST, "Entity Item Blacklist (allows you to blacklist items/blocks for entities that can pick up things). Any changes take effect after a Minecraft restart.");
		...
		odbExplosionSize = cfg.get(CATEGORY_SILLY_SETTINGS, "ODB Explosion Size", 160, "The explosion size of an ODB. 400 is the rough limit if running on 2GB of RAM, anything above that will require more allocated memory, and could crash the game or freeze it for longer periods of time.", 80, 800).getInt();
		antimatterExplosionSize = cfg.get(CATEGORY_SILLY_SETTINGS, "Antimatter Explosion Size", 80, "The explosion size of antimatter mobs colliding with their normal counterpart if Nucler Antimatter Explosions is enabled.", 40, 200).getInt();
