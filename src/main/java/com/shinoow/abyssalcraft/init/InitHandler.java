...
public class InitHandler implements ILifeCycleHandler {
	...
	public static final Map<String, Tuple<Integer, Float>> demon_transformations = Maps.newHashMap();
	...
	private static void syncConfig(){
		...
		shoggothLairSpawnRate = MathHelper.clamp_int(shoggothLairSpawnRate, 0, 1000);
		...
		acidResistanceHardness = MathHelper.clamp_double(acidResistanceHardness, 3.0D, 51.0D);
		...
		for(String str : transformations)
			if(str.length() > 0){
				...
				if(stuff.length >= 2)
					demon_transformations.put(stuff[0], new Tuple(Integer.valueOf(stuff[1]), stuff.length == 3 ? Float.valueOf(stuff[2]) : 1));
