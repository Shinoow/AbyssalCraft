...
public class InitHandler implements ILifeCycleHandler {
	...
	private static void syncConfig(){
		...
		nightVisionEverywhere = cfg.get(Configuration.CATEGORY_GENERAL, "Plated Coralium Helmet Night Vision Everywhere", false, "Toggles whether or not the Night Vision buff from the Plated Coralium Helmet should be applied in all dimensions, rather than only Surface Worlds.").getBoolean();
		...
		monolithBuildingCooldown = cfg.get("shoggoth", "Monolith Building Cooldown", 1800, "The cooldown (in ticks) between each attempt by a Lesser Shoggoth to construct a monolith. Higher values increase the time, while lower values decrease it (and 0 disables it).\n[range: 0 ~ 2400, default: 1800]", 0, 2400).getInt();
		...
		shoggothLairSpawnRate = cfg.get("worldgen", "Shoggoth Lair Generation Chance: Swamps", 30, "Generation chance of a Shoggoth Lair in swamp biomes. Higher numbers decrease the chance of a Lair generating, while lower numbers increase the chance.\n[range: 0 ~ 1000, default: 30]", 0, 1000).getInt();
